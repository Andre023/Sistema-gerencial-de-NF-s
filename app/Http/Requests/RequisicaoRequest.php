...
class RequisicaoRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $rules = [
            ...
            'loja'          => $obrig('integer|in:' . implode(',', Requisicao::LOJAS)),
            'motivo'        => $obrig('string|in:' . implode(',', Requisicao::MOTIVOS)),
            'observacao'    => 'nullable|string|max:500',
        ];

        // Status só é aceito na edição — na criação é sempre "Pendente".
        if ($this->isMethod('patch')) {
            $rules['status'] = 'sometimes|string|in:' . implode(',', Requisicao::STATUS);
        }
